...
- 🔴 **Преленд/ленд (блек) — НИКОГДА в default path. Только rules-path, цепляет арб вручную.** Два случая по состоянию кампании вайта в Бином:
  - **Кампания УЖЕ создана** (сайт настроен, есть ленды/интеграция) → кампанию **не создавать и default path вообще НЕ ТРОГАТЬ**. Скилл только создаёт преленд (`landing_id` из 4c) + оффер-ленд (`offer_id` из 4d) — арб сам цепляет их в rules.
  - **Кампании НЕТ** (первая интеграция вайта) → создать кампанию вайта (4e) и в **default path положить landing = скомпилированную копию главной вайта** (браузерный View Source, не PHP) + **offer = `https://<white>`**. Без landing в default кампания в бином **не создастся** — обязательный шаг создания, а не «белый ленд для клоаки».
...
- **(1я настройка) создание кампании вайта:** default path = белая копия (landing) + offer по правилу выше. Детали — `integration-m2-dao/references/binom-config.md`. Константы `page.php` (EU): `b2euro.com/sucsess` + `API_KEY=f6802a221...` (шаблон `integrations/m2/eur/DAO/page.php` исправлен). (Финальная сборка преленд+ленд в rules — отдельно, см. примечание выше.)
