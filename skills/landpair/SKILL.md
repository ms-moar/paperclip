...
- 🔴 **Default path кампании вайта = ВСЕГДА скомпилированная копия главной вайта, НИКОГДА преленд/ленд.** Default path неизменно: **landing = `index.html` со СКОМПИЛИРОВАННЫМ исходником вайта** (браузерный View Source, не PHP) + **offer = `https://<white>`**. Преленд (integrated, `landing_id` из 4c) и оффер-ленд (`offer_id` из 4d) идут ТОЛЬКО в **rules-path** — цепляет арб вручную. Правило действует ВСЕГДА, не только при 1й настройке. Причина: Google-модератор/бот открывает кампанию без клоак-контекста и попадает в default → там обязана быть белая страница, иначе палево блека.
- **(1я настройка) создание кампании вайта:** если ещё не создана — создать с Default path (белая копия + offer) по правилу выше. Детали — `integration-m2-dao/references/binom-config.md`. Константы `page.php` (EU): `b2euro.com/sucsess` + `API_KEY=f6802a221...` (шаблон `integrations/m2/eur/DAO/page.php` исправлен). (Финальная сборка преленд+ленд в rules — отдельно, см. примечание выше.)
